...
+time(_): mypos(X,Y) & Y > 21 & food(Food) & .min(Food,[_,_,Fx,Fy]) & agent(Agent) & .min(Agent,[_,_,_,_,Fx,Fy,_]) & target(_,_) <-
	-target(_,_);
	!move(0).
...
+time(_): mypos(X,Y) & Y < 10 & food(Food) & .min(Food,[_,_,Fx,Fy]) & agent(Agent) & .min(Agent,[_,_,_,_,Fx,Fy,_]) & target(_,_) <-
	-target(_,_);
	!move(2).
